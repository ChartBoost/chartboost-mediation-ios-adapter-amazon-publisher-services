...
final class AmazonPublisherServicesAdapter: PartnerAdapter {
    ...
    func setConsents(_ consents: [ConsentKey: ConsentValue], modifiedKeys: Set<ConsentKey>) {
        ...
        if modifiedKeys.contains(configuration.partnerID) || modifiedKeys.contains(ConsentKeys.gdprConsentGiven) {
            ...
            case ConsentValues.granted:
                ...
                DTBAds.sharedInstance().setConsentStatus(.EXPLICIT_YES)
                log(.privacyUpdated(setting: "consentStatus", value: "EXPLICIT_YES"))
            case ConsentValues.denied:
                DTBAds.sharedInstance().setCmpFlavor(.CMP_NOT_DEFINED)
                DTBAds.sharedInstance().setConsentStatus(.EXPLICIT_NO)
                log(.privacyUpdated(setting: "consentStatus", value: "EXPLICIT_NO"))
            default:
                break
            }
        }
    ...
    func setIsUserUnderage(_ isUserUnderage: Bool) {
        ...
        // The Children’s Online Privacy Protection Act (COPPA) is a United States federal law that is designed to give parents control 
        // over the information collected from their young children online. COPPA prohibits the collection, use, or disclosure of personal
        // information from children under 13 unless you have provided notice and received their parent’s consent. Parental consent must be
        // verifiable (e.g., having the parent fill out a consent form, or checking the parent’s government-issued ID).
        //
        // Apps that are directed at children under 13 are not eligible to participate in TAM nor work with APS. This is because of 
        // restrictions on advertising found in COPPA and in related regulations relating to online advertising. You are responsible for
        // complying with the restrictions to allow advertisement to be served on a child directed app and, when applicable to your app,
        // with COPPA. COPPA is enforced by the FTC and the penalties for violating COPPA can be as high as $16,000 per violation.
        //
        // Even if your app is directed at a mixed audience, including people both over and under the age of 13, you may not show ads from 
        // APS to users you know are under 13. This applies equally even in an app that is not child-directed. For example, if you ask a
        // user for their age and they indicate they are under 13, you may not show an ad to them that you source from the APS integration
        // and/or TAM.
...
extension String {
    ...
    fileprivate static let prebidsKey = "prebids"
    fileprivate static let managedPrebiddingKey = "managed_prebidding"
}
...
extension PartnerConfiguration {
    ...
    fileprivate var preBidSettings: [String: AmazonPublisherServicesAdapterPreBidRequest.AmazonSettings] {
    ...
    fileprivate var useManagedPreBidding: Bool {
